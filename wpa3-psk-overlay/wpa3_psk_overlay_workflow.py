...
def create_wpa3_psk(central_conn, ssid_details):
    """
...
def assign_wpa3_psk_to_site(central_conn, site_id, ssid_name):
    """
...
def change_wpa3_psk_passphrase(central_conn, passphrase_file):
    """
    Changes the WPA3 PSK passphrase for a specified SSID.

    :param central_conn: Instance of class:`pycentral.NewCentralBase` to establish connection to Central.
    :type central_conn: class:`NewCentralBase`
    :param passphrase_file: Path to the YAML file containing the new passphrase and SSID name.
    :type passphrase_file: str
    """
    # Load the new passphrase and SSID name from the YAML file
    passphrase_data = yaml.safe_load(open(passphrase_file, "r"))
    ssid_name = passphrase_data["ssid_name"]
    new_passphrase = passphrase_data["new_passphrase"]

    # Construct the payload for updating the passphrase
    payload = {
        "ssid": ssid_name,
        "personal-security": {
            "passphrase-format": "STRING",
            "wpa-passphrase": new_passphrase
        }
    }

    # Use the Wlan.update_wlan method to update the passphrase
    result = Wlan.update_wlan(central_conn=central_conn, config_dict=payload)

    # Check the result and print appropriate messages
    if result:
        print(f"Successfully updated passphrase for SSID: {colored(ssid_name, 'green')}")
        # Display the table with PSK name and new passphrase
        print(tabulate([[ssid_name, new_passphrase]], headers=["PSK Name", "New Passphrase"], tablefmt="grid"))
    else:
        print(f"Error updating passphrase for SSID: {colored(ssid_name, 'red')}")
        exit()

def generate_and_update_wpa3_psk_passphrase(central_conn, passphrase_file):
    """
    Generates a new WPA3 PSK passphrase, updates the WLAN profile, and writes the passphrase to a file.

    :param central_conn: Instance of class:`pycentral.NewCentralBase` to establish connection to Central.
    :type central_conn: class:`NewCentralBase`
    :param passphrase_file: Path to the YAML file to save the generated passphrase.
    :type passphrase_file: str
    """
    passphrase_data = yaml.safe_load(open(passphrase_file, "r"))
    ssid_name = passphrase_data["ssid_name"]
    ...
    new_passphrase = ''.join(random.choices(string.ascii_letters + string.digits, k=16))
    ...
    if result:
        print(f"Successfully updated passphrase for SSID: {colored(ssid_name, 'green')}")
        # Write the new passphrase to the YAML file
        with open(passphrase_file, "w") as file:
            yaml.dump({"ssid_name": ssid_name, "new_passphrase": new_passphrase}, file)
        print(f"New passphrase saved to {passphrase_file}")
        # Display the table with PSK name and new passphrase
        print(tabulate([[ssid_name, new_passphrase]], headers=["PSK Name", "New Passphrase"], tablefmt="grid"))
    else:
        print(f"Error updating passphrase for SSID: {colored(ssid_name, 'red')}")
...
def handle_passphrase_steps(central_conn, passphrase_file):
    """
    Handles the conditional execution of passphrase update steps.
    ...
    """
    passphrase_data = yaml.safe_load(open(passphrase_file, "r"))
    ssid_name = passphrase_data.get("ssid_name")
    new_passphrase = passphrase_data.get("new_passphrase")

    if not ssid_name:
        print(f"Error: SSID name is missing in the passphrase file.")
        exit()

    if new_passphrase:
        print("Step 13: Change WPA3 PSK Passphrase")
        if confirm_action(f"Proceed with caution, this will change the passphrase for WPA3 PSK '{ssid_name}'. Do you want to continue?"):
            change_wpa3_psk_passphrase(central_conn, passphrase_file)
        else:
            print("Step 13 skipped.")
    else:
        print("Step 14: Generate and change WPA3 PSK Passphrase")
        if confirm_action(f"Proceed with caution, this will generate and change the passphrase for WPA3 PSK '{ssid_name}'. Do you want to continue?"):
            generate_and_update_wpa3_psk_passphrase(central_conn, passphrase_file)
        else:
            print("Step 14 skipped.")
...
def parse_args():
    ...
    parser.add_argument(
        "-np",
        "--new_passphrase",
        help="Path to the new passphrase file (must be YAML format)",
        required=False,
        type=validate_file_format,
    )
...
def main():
    ...
    print("Step 9: Create WPA3 PSK")
    create_wpa3_psk(central_conn, profiles_vars["ssid_details"])

    print("Step 10: Assign WPA3 PSK to Site")
    assign_wpa3_psk_to_site(central_conn, site_id, profiles_vars["ssid_details"]["wlan-ssid"][0]["ssid"])
    ...
    if args.new_passphrase:
        handle_passphrase_steps(central_conn, args.new_passphrase)
