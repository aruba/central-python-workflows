"""
Sample python script with functions to create/update/delete groups, sites, templates, variables and move devices to a group 
"""
...
class ArubaCentralConfiguration:
    ...
    def isExists(self, apiPath):
        ...
        try:
            ...
            print("***Exception: Method: %s; Error: %s; apiPath: %s" % ("GET", err, apiPath))
    ...
    def delete(self, apiPath):
        apiMethod = "DELETE"
        try:
            resp = self.deviceHandle.command(apiPath=apiPath, apiMethod=apiMethod)
            if resp != '':
                if resp["code"] == 200 or resp["code"] == 201:
                    return True
                else:
            ...
            print("***Exception: Method: %s; Error: %s; apiPath: %s" % (apiMethod, err, apiPath))

    def createGroup(self, groupName, groupPass="admin1234", templateGroup={"Wired":True, "Wireless":True}):
        apiData = ""
        ...
        if self.isExists(apiPath=apiPath) == "PATCH":
            if templateGroup == False:
                apiData = {"group_password": groupPass, "template_group": templateGroup}
            ...
            apiData = {"group": groupName,  "group_attributes": {"group_password": groupPass, "template_info": templateGroup}}
            
        resp = self.deviceHandle.command(apiMethod=apiMethod, apiPath=apiPath, apiData=apiData)
        if resp != '':
            if resp["code"] == 200 or resp["code"] == 201:
    ...
    def uploadTemplate(self, groupName, templateName, deviceType, fileName, version="ALL", model="ALL"):
        apiPath = "/configuration/v1/groups/" + groupName +"/templates/" + templateName
        ...
        resp = ''
        apiPath = "/configuration/v1/groups/" + groupName +"/templates"
        apiParams = {"name": templateName, "device_type": deviceType, "version": version, "model": model}
        resp = self.deviceHandle.command(apiMethod=apiMethod, apiPath=apiPath, apiParams=apiParams, files=files, headers="")
        if resp != '':
            if resp["code"] == 200 or resp["code"] == 201:
                print("Success: template %s added to group %s" % (templateName, groupName))
            else:
                print("Status Code: %d" % resp["code"])
                print("Message: %s" % resp["msg"])
    ...
    def deleteTemplate(self, groupName, templateName):
        apiPath = "/configuration/v1/groups/"+ groupName +"/templates/" + templateName
        if self.delete(apiPath=apiPath):
            print("Deleted template %s from group %s" % (templateName, groupName))

    def uploadVariable(self, fileName=None, device_serial=None, variables=None):
        #Expects template variables or file in JSON format. If fileName is not provided, (device_serial and variables) are expected.
        #Sample variable file in JSON format can be downloaded from central
        #if device_serial is provided in the argument, variables to be updated for that device should be provided in JSON format
        ...
        if fileName:
            ...
            try:
                ...
                raise
            #Uploading template variables as a file does not throw an error when device is not present under the customer
            ...
            for device_serial, variables in fp_dict.items():
                ...
                apiPath = "/configuration/v1/devices/" + device_serial + "/template_variables"
                if self.isExists(apiPath):
                    apiMethod = "PATCH"
                responses.append(self.deviceHandle.command(apiMethod=apiMethod, apiPath=apiPath, apiData=apiData))

        elif device_serial:
            if not variables:
                print("Exception.. Skipping variable update:: Provide variables in JSON format for this device %s" % device_serial)
            else:
                apiPath = "/configuration/v1/devices/" + device_serial + "/template_variables"
                apiData = {"variables": variables}
                if self.isExists(apiPath):
    ...
    def deleteVariablesOfDevice(self, deviceSerial):
        apiPath = "/configuration/v1/devices/" + deviceSerial + "/template_variables"
        if self.delete(apiPath=apiPath):
    ...
    def findSiteId(self, siteName):
        ...
        resp = self.deviceHandle.command(apiMethod=apiMethod, apiPath=apiPath, apiData=apiData)
        successMsg = "Checking list of sites to match with the siteName " + siteName
    ...
    def createSite(self, siteName, siteAddress):
        ...
        if siteId:
            ...
            apiMethod = "PATCH"
        apiData = { "site_name": siteName, "site_address": siteAddress }
        resp = self.deviceHandle.command(apiMethod=apiMethod, apiPath=apiPath, apiData=apiData)
    ...
    def addDevicesToSite(self, deviceSerials, deviceType, siteId=None, siteName=None):
        """
        ...
        if not siteId:
            if siteName:
                ...
                return False
        if siteId:
            apiData = { "device_ids": deviceSerials, "device_type": deviceType, "label_id": siteId }
            resp = self.deviceHandle.command(apiMethod=apiMethod, apiPath=apiPath, apiData=apiData)
            successMsg = "Added devices to the site" + str(siteId)
            return self.checkResponse(resp, successMsg)

    def removeDevicesFromSite(self, deviceSerials, deviceType, siteId=None, siteName=None):
        """
        deviceSerials: Type list (required)
        siteId: Type integer (if optional, siteName is required)
        siteName: Type string (if optional, siteId is required)
        ...
        if not siteId:
            if siteName:
                ...
                print("Provide siteId or siteName to delete device from the site")
                return False
        if siteId:
            apiData = { "device_ids": deviceSerials, "device_type": deviceType, "label_id": siteId }
            resp = self.deviceHandle.command(apiMethod=apiMethod, apiPath=apiPath, apiData=apiData)
    ...
    def modifyUser(self, apiData, apiParams):
        ...
        apiMethod = "PATCH"
        resp = self.deviceHandle.command(apiPath=apiPath, apiData=apiData, apiMethod=apiMethod)
